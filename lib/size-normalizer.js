...
const JEWELRY_TYPES = [
    'maang tikka', 'earrings', 'necklace', 'bracelet', 'bangle',
    'pendant', 'anklet', 'nath', 'chain', 'ring',
];

export function identifyJewelryType(piece) {
    const typeStr = (piece?.type || '').toLowerCase();
    const descStr = (piece?.description || '').toLowerCase();
    const sorted = [...JEWELRY_TYPES].sort((a, b) => b.length - a.length);

    for (const type of sorted) {
        const pattern = new RegExp(`\\b${type}\\b`, 'i');
        if (pattern.test(typeStr) || pattern.test(descStr)) return type;
    }
    for (const type of sorted) {
        if (typeStr.includes(type) || descStr.includes(type)) return type;
    }
    return typeStr || 'jewelry';
}
