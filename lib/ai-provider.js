...
function buildPublicImageUrl(filename) {
    ...
    if (!baseUrl || isLocalUrl(baseUrl)) return null;
    return `${baseUrl}/api/images?filename=${encodeURIComponent(filename)}`;
}

async function uploadToCatbox(base64Data, mimeType = 'image/jpeg') {
    ...
    formData.append('fileToUpload', new File([buffer], `image.${extension}`, { type: mimeType }));
...
async function resolveReferenceImageUrl(base64, mimeType = 'image/jpeg') {
    const publicUrl = buildPublicImageUrl(saveReferenceImage(base64, mimeType));
    if (publicUrl) {
        console.log(`Reference image hosted at ${publicUrl}`);
        return publicUrl;
    }

    console.log('No public base URL configured, falling back to Catbox upload');
    return uploadToCatbox(base64, mimeType);
}
...
async function uploadReferenceImages(images) {
    ...
    for (let i = 0; i < images.length; i++) {
        ...
        const url = await resolveReferenceImageUrl(img.base64, mimeType);
        uploaded.push({
            image: url,
            mime_type: mimeType,
