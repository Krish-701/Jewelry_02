...
function buildStrictSizePrompt(sizeValidations) {
    ...
    if (validSizes.length === 0) {
        return { sizeBlock: '', sizeConstraints: '', compactSizeBlock: '' };
    }

    const sizeSummary = validSizes
        .map((sv) => `${sv.pieceName}: ${sv.numericValue} ${sv.unit}`)
        .join('; ');

    const compactSizeBlock = buildCompactSizeBlock(validSizes);

    return {
        sizeBlock: compactSizeBlock,
        sizeConstraints: `Sizes: ${sizeSummary}. `,
        compactSizeBlock,
    };
}
...
export function verifyAndBuildPrompt({
    ...
    const { sizeBlock, sizeConstraints, compactSizeBlock } = buildStrictSizePrompt(sizeValidation);

    const promptParts = [];

    promptParts.push(
        'Ultra-photorealistic jewelry advertisement photograph, 8K, Hasselblad macro lens, tack-sharp jewelry focus'
    );
    ...
    if (hasPieces || imageCount > 1) {
        ...
        promptParts.push(
            ...
            'photographic accuracy on every detail'
        );
    } else {
        promptParts.push(
            ...
            'jewelry is the hero of the shot'
        );
    }

    if (sizeBlock) promptParts.push(sizeBlock);
