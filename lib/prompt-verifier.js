...
import { buildEnforcedSizeBlock, identifyJewelryType, normalizeJewelrySize } from './size-normalizer.js';
import { buildAdaptiveJewelryPrompt } from './adaptive-prompt.js';
...
export function verifyAndBuildPrompt({
    ...
    const { compactSizeBlock, scaleHints: userScaleHints } = buildStrictSizePrompt(sizeValidation);
    const adaptive = buildAdaptiveJewelryPrompt(analysis, sizes, sizeValidation);

    const promptParts = [];

    promptParts.push(adaptive.focusBlock);

    const scaleBlock = adaptive.compactScaleBlock || compactSizeBlock;
    if (scaleBlock) promptParts.push(scaleBlock);

    if (sessionMemory) promptParts.push(sessionMemory);

    const poseHints = adaptive.focusPieces.map((p) => p.placement.pose).join(', ');
    promptParts.push(
        `Ultra-photorealistic jewelry advertisement photograph, 8K, Hasselblad macro lens, tack-sharp focus on ${adaptive.pieceNames}. ` +
        `Pose guidance: ${poseHints}. Match reference jewelry exactly — correct scale on model`
    );
    ...
    promptParts.push(
        `${jewelryDesc}. ${material}${style ? ` ${style}` : ''}${stones ? ` with ${stones}` : ''}${finish ? `, ${finish} finish` : ''} ${multiRef}. ` +
        `Reproduce every detail from reference. ONLY show: ${adaptive.pieceNames}. No extra jewelry.`
    );
    ...
    return {
        ...
        scaleHints: adaptive.scaleHints.length ? adaptive.scaleHints : userScaleHints,
        focusPieces: adaptive.focusPieces.map((p) => p.name),
    };
}
